...
6. **Tool Competence — ACTIVE; INITIAL SCENARIOS COMPLETE**
   Local files, databases, n8n, allowlisted Python handlers, read-only browser
...
12. **External Communities — BLOCKED / LATE**
    Requires mature identity, security, authority, and learning governance.
...
Current bounded curriculum: `FOUNDATIONAL_CURRICULUM.json` version 0.2.0.
Seven units and eight scenarios are persisted with exact source hashes,
append-only versions, read-only drift detection, and untrusted local-model
assessment records. The guided local engine scored 5/8 exact and remains
`needs_review`; deterministic policy remains the enforcement layer. Learning
does not grant execution authority; all records state `capability_change: none`.
