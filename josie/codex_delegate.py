...
def _public(result: dict) -> dict:
    ...
    result['assistant_message'] = (
        'JOSIE CODEX DELEGATION — ACTUAL RESULT\n'
...
def delegation_status(project_root: Path, request_id: str) -> dict:
    ...
    return _public(json.loads(path.read_text(encoding='utf-8')))
...
def delegate_codex(user_request: str, *, request_id: str, project_root: Path) -> dict:
    ...
    if executable is None:
        return _public({'request_id': request_id, 'status': 'unavailable',
                        'error': 'Authenticated local Codex CLI not found'})
    ...
    result = {'request_id': request_id, 'task_sha256': task_hash,
