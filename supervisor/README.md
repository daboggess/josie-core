...
## Knowledge + Priming (Phase 1 & Phase 2)
...
5. **Canonical Knowledge Integration (Phase 2)**: `josie/knowledge.py` connects real Josie canon to priming and Prompt Contracts:
   - Queries and filters real structured knowledge (`KnowledgeRecord`, `KnowledgeQuery`).
   - Distinguishes active/canonical records from superseded and rejected records with explicit provenance.
   - Idempotently integrates with SQLite `memory_claims`, `claim_evidence`, and `entities` in `data/josie.db` without schema alterations or storage duplication.
   - Assembles immutable priming bundles directly from canonical knowledge records via `assemble_priming_from_knowledge()`.

Run deterministic qualification and knowledge tests without an LLM:

```powershell
$env:PYTHONDONTWRITEBYTECODE='1'
python -m unittest tests.test_knowledge supervisor.tests.test_priming supervisor.tests.test_prompt_contract supervisor.tests.test_supervisor -v
```
